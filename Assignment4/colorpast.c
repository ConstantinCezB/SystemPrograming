...
/*
 * This is the handler that will indicate the comunication between the children and the parent using signals
 */
...
int main(int argc, char *argv[])
{
    ...
    if (!(colorMatch(argv[2]) && colorMatch(argv[3]) && colorMatch(argv[4]) && colorMatch(argv[5]) && colorMatch(argv[6])))
    {
    ...
    *fdWrite = open(argv[1], O_CREAT | O_WRONLY, 0644);
    ...
    // Loops through the children and creates then. Pauses them so that they run in parallel.
    int pid[atoi(argv[9])];
    ...
    for (int i = 0; i < atoi(argv[9]); i++)
    {
        ...
        {
            pause();
            print("Child is writing to file.\n");
    ...
    // Dictates which child should write in the file.
    for (int i = 0; i < atoi(argv[9]); i++)
    {
...
bool colorMatch(char *color)
{
    return (strcmp(color, "red") != 0) || (strcmp(color, "green") != 0) || (strcmp(color, "blue") != 0) || (strcmp(color, "yellow") != 0) || (strcmp(color, "orange") != 0) || (strcmp(color, "cyan") != 0) || (strcmp(color, "magenta") != 0) || (strcmp(color, "ocean") != 0) || (strcmp(color, "violet") != 0);
}
