...
void readPictureInfo (int *fd, char picFormat[10], char picX[10], char picY[10], char picScale[10]) {
    ...
    while ((stage < 3) && (read(*fd, buf, 1) > 0)){
        ...
        {
            if(reading){
                if(stage == 0) picFormat[indexFormat] = '\0';
                else if(stage == 1) picY[indexFormat] = '\0';
                else if(stage == 2) picScale[indexFormat] = '\0';
                indexFormat = 0;
                stage++;
            }
            reading = true; 
        ...
        {
            if(stage == 0 ) picFormat[indexFormat++] = buf[0];
            else if(stage == 1)
            {
                if(buf[0] == ' '){
                    picX[indexFormat] = '\0';
                ...
                if(isDigit(buf[0])){
                    if(pixelReadingStagePart) picX[indexFormat++] = buf[0];
                    else picY[indexFormat++] = buf[0];
                }
            }
            else if(stage == 2 && isDigit(buf[0])) picScale[indexFormat++] = buf[0];
        }
    }
...
void createMerge (int *fd1, int *fd2, int *fdWrite, char format[],char pic1X[10], char pic1Y[10], char pic1Scale[10], char pic2X[10], char pic2Y[10]){
    ...
    char buf[3];
    bool picChange = true;
    int pic1XInt = atoi(pic1X), pic1YInt = atoi(pic1Y), pic2XInt = atoi(pic2X), pic2YInt = atoi(pic2Y), pic1Write = pic1XInt - pic2XInt;
    int index = 1;
